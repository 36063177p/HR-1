...
// دالة حساب الوقت بدقة (ساعات:دقائق:ثواني)
function calculatePreciseTime(startTime, endTime, date) {
    ...
    const result = {
        totalMs: diffMs,
        totalHours: diffMs / (1000 * 60 * 60), // عدد الساعات كرقم عشري
        display: `${hours.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`
    };
    
    console.log('⏱️ نتيجة حساب الوقت:', result);
    return result;
}
...
function generateWeeklyReport() {
    ...
    employees.filter(emp => emp.role !== 'manager').forEach(employee => {
        ...
        // حساب الراتب بالساعة (الطريقة الجديدة: الراتب ÷ 6 أيام ÷ 10.5 ساعة)
...
function loadEmployeeFinancialReport() {
    ...
    weekAttendance.forEach(att => {
        if (att.checkIn && att.checkOut) {
            ...
            totalEarnings += timeCalc.totalHours * hourlyRate;
            workDays++;
        }
    });
        ...
        if (isWeekend) {
            ...
            if (attendanceRecord.checkIn) {
                ...
                if (attendanceRecord.checkOut) {
                    ...
                    dayValue = (timeCalc.totalHours * hourlyRate).toFixed(2);
                    status = '✅ حضور كامل';
                } else {
                    status = '⚠️ لم ينصرف';
                }
